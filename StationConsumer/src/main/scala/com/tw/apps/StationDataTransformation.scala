...
object StationDataTransformation {
    ...
    stations.asInstanceOf[Seq[Map[String, Any]]]
      .map(x => {
        StationData(
          ...
          extractBooleanValueIfExist( x("extra").asInstanceOf[Map[String, Any]],"renting"),
          extractBooleanValueIfExist( x("extra").asInstanceOf[Map[String, Any]],"returning"),
  ...
  private def extractBooleanValueIfExist(data: Map[String, Any], key: String): Boolean = {
